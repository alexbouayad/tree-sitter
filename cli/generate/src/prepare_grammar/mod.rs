...
use super::{
    grammars::{
        ...
        Production, ProductionStep, ReservedWordContext, ReservedWordSetId, SyntaxGrammar,
        SyntaxVariable, Variable, VariableType,
    },
...
pub fn prepare_grammar(
    ...
    if extend_grammar {
        let mut external_tokens = syntax_grammar.external_tokens.clone();
        let mut syntax_variables = Vec::new();

        for (variable_index, variable) in syntax_grammar.variables.iter().enumerate() {
            if variable_index == 0 {
                syntax_variables.push(variable.clone());
                continue;
            }
            ...
            let mut productions = variable.productions.clone();

            for production in &variable.productions {
                let mut steps = Vec::new();

                steps.push(ProductionStep {
                    symbol: Symbol::external(external_tokens.len() - 2),
                ...
                steps.extend(production.steps.iter().cloned());

                steps.push(ProductionStep {
                    symbol: Symbol::external(external_tokens.len() - 1),
                    precedence: Precedence::NoPrecedence(),
                    associativity: None,
                    alias: None,
                    field_name: None,
        ...
        syntax_grammar.external_tokens = external_tokens;
        syntax_grammar.variables = syntax_variables;
    }
