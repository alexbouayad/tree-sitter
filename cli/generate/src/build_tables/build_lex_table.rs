...
pub fn build_lex_table(
    ...
    let mut builder = LexTableBuilder::new(lexical_grammar);
    for (tokens, parse_state_ids) in parse_state_ids_by_token_set.iter() {
    ...
    if let Some(word_token) = syntax_grammar.word_token {
        let word_lex_state_id = builder.add_state_for_tokens(&TokenSet::from_iter([word_token]));

        for (tokens, parse_state_ids) in parse_state_ids_by_token_set.iter() {
            if !tokens.contains(&word_token) && tokens.iter().any(|t| keywords.contains(&t)) {
                for id in parse_state_ids {
                    parse_table.states[*id].anti_lex_state_id = Some(word_lex_state_id);
                }
            }
        }
    }
...
fn minimize_lex_table(table: &mut LexTable, parse_table: &mut ParseTable) {
    ...
    for state in &mut parse_table.states {
        ...
        state.anti_lex_state_id = state.anti_lex_state_id.map(|id| group_ids_by_state_id[id]);
    }
...
fn sort_states(table: &mut LexTable, parse_table: &mut ParseTable) {
    ...
    for state in &mut parse_table.states {
        ...
        state.anti_lex_state_id = state.anti_lex_state_id.map(|id| new_ids_by_old_id[id]);
    }
}
