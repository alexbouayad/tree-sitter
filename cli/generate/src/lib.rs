...
pub fn rename_variables(
    syntax_grammar: &mut grammars::SyntaxGrammar,
    lexical_grammar: &mut grammars::LexicalGrammar,
) {
    for variable in &mut syntax_grammar.variables {
        // if variable.kind == grammars::VariableType::Auxiliary {
        //     variable.name = format!("@{}", variable.name);
        // }

        if variable.kind == grammars::VariableType::Hidden {
            variable.kind = grammars::VariableType::Named;
        }
    }

    for variable in &mut syntax_grammar.external_tokens {
        variable.kind = grammars::VariableType::Named;
    }

    for variable in &mut lexical_grammar.variables {
        // if variable.kind == grammars::VariableType::Auxiliary {
        //     variable.name = format!("@{}", variable.name);
        // }
...
fn generate_parser_for_grammar_with_opts(
    ...
    let (mut syntax_grammar, mut lexical_grammar, inlines, simple_aliases) =
        prepare_grammar(input_grammar, false)?;
